...
export const fonts2Style = () => {
  let fontsFileStyle = `${app.path.srcFolder}/scss/fonts.scss`;

  fs.readdir(app.path.build.fonts, function (error, fontsFile) {
    if (fontsFile) {
      if (!fs.existsSync(fontsFileStyle)) {
        fs.writeFile(fontsFileStyle, "", cb);
        let newFileOnly;
